...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (body.quality > 5 || body.quality < 0 || body.fit > 5 || body.fit < 0
    || (body.team && (body.team > 5 || body.team < 0)) || (body.investors && (body.investors > 5 || body.investors < 0))
  ) {
    return NextResponse.json(
      { success: false, message: 'Invalid ratings format. Quality and Fit are required as numbers. Everything must be between 0 and 5' },
      { status: 400 }
    );
  }
