...
import { Investment, FundingRound, Investor, PrismaClient } from '@prisma/client';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const groupedFundingRounds = fundingRounds.reduce((acc: { [key: string]: (Investment & { fundingRound: FundingRound } & { investor: Investor })[] }, current) => {
        ...
        const mappedFundingRounds = Object.values(groupedFundingRounds).sort((a, b) => new Date(b[0]?.fundingRound.date ?? '').getTime() - new Date(a[0]?.fundingRound.date ?? '').getTime()).map((group) => {
            const round = group[0]?.fundingRound.round;
            const date = group[0]?.fundingRound.date;
            const amount = group[0]?.fundingRound.amount;
            const leads = group.filter((investment) => investment.isLead).map((investment) => mapInvestor(investment.investor));
            const followers = group.filter((investment) => !investment.isLead).map((investment) => mapInvestor(investment.investor));
        ...
        const company = {
            ...
            categories: categories.filter((category) => category.category.categoryType === 1)
                .map((category) => category.category.label),
            categoryGroups: categories.filter((category) => category.category.categoryType === 2)
                .map((category) => category.category.label),
